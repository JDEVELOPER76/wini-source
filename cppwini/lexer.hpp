...
private:
    ...
    // Contexto de análisis (para saber si estamos declarando una constante)
    enum class Contexto {
        NORMAL,
        ...
        if (it != palabras_clave.end()) {
            ...
            if (tipo_token == Token::Tipo::TIPO_ENTERO ||
                ...
                if (contexto_actual == Contexto::DECLARACION_CONSTANTE) {
                    // Estamos en una declaración, el siguiente token debe ser un identificador
                }
            ...
            // Si estamos en contexto de declaración, el identificador es una variable
            if (contexto_actual == Contexto::DECLARACION_CONSTANTE) {
                token.es_constante = (contexto_actual == Contexto::DECLARACION_CONSTANTE);
